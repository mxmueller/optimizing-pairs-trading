...
class PCIProcessor:
    ...
    def estimate_parameters(self, x1, x2):
        ...
        result = minimize(
            neg_log_likelihood,
            x0=[beta_init, 0.9, std_init * 0.5, std_init * 0.5],  # angepasste Startwerte
            bounds=[
                (-2, 2),  # beta
                (0.6, 0.99),  # rho
                (0.1, 1),  # sigma_m
    ...
    def kalman_filter(self, x1, x2):
        ...
        for t in range(n):
            wt = x2_norm[t] - self.beta * x1_norm[t]  # Spread
            if t > 0:
                pred_mt = self.rho * mt[t - 1]
                pred_rt = rt[t - 1]
                et = wt - (pred_mt + pred_rt)  # Innovation

                # Update mit fixem Kalman Gain

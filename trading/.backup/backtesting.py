...
class PairBacktest:
    ...
    def check_stop_loss(self, row):
        trade_data = self.market_data[
            (self.market_data['symbol'] == row['symbol']) & 
            (self.market_data['date'] > row['entry_date']) & 
            (self.market_data['date'] <= row['exit_date'])
        ]
        
        if trade_data.empty:
            return row['exit_price'], row['exit_type'], row['exit_date']

        stop_price = (row['entry_price'] * (1 - self.config.stop_loss_pct) 
                     if row['position_type'] == 'long' 
                     else row['entry_price'] * (1 + self.config.stop_loss_pct))

        for _, price_row in trade_data.iterrows():
            if (row['position_type'] == 'long' and price_row['low'] <= stop_price) or \
               (row['position_type'] == 'short' and price_row['high'] >= stop_price):
                return stop_price, 'stop_loss', price_row['date']
    ...
    def calculate_trade_pnl(self):
        ...
        for idx in self.trades_pnl.index:
            exit_price, exit_type, exit_date = self.check_stop_loss(self.trades_pnl.loc[idx])
            self.trades_pnl.at[idx, 'exit_price'] = exit_price
            self.trades_pnl.at[idx, 'exit_type'] = exit_type
            self.trades_pnl.at[idx, 'exit_date'] = exit_date
...
class MultiStrategyBacktest:
    def __init__(self, config=None, market_data_path='../notebooks/sim_daily.parquet'):
        self.config = config if config else BacktestConfig()
